...
print("=" * 55)
print("STEP 2 — INCREMENTAL STAGING LAYER: Dam Levels")
print("=" * 55)
...
# 2. Fix date column
df.columns = [c.replace('iDATE', 'DATE') for c in df.columns]
...
# 11. Fill nulls in numeric columns
numeric_cols = ['height_m', 'storage_ml', 'current_pct', 'last_year_pct',
                'big6_storage_ml', 'big6_current_pct', 'big6_last_year_pct']
for col in numeric_cols:
    long_df[col] = pd.to_numeric(long_df[col], errors='coerce').fillna(0).round(2)

long_df['is_critical'] = long_df['is_critical'].astype(int)
long_df['is_day_zero'] = long_df['is_day_zero'].astype(int)

# ── INCREMENTAL LOGIC ──────────────────────────────────────────────────────
print("\n[3] Checking for existing staging data...")

if os.path.exists(STAGING_PATH):
    existing_df = pd.read_csv(STAGING_PATH)
    existing_df['date'] = pd.to_datetime(existing_df['date'])
    latest_date = existing_df['date'].max()
    print(f" Existing staging found : {len(existing_df):,} rows")
    print(f" Latest date in staging : {latest_date.date()}")

    # Only keep new rows not already in staging
    new_rows = long_df[long_df['date'] > latest_date]
    print(f" New rows to append : {len(new_rows):,}")

    if len(new_rows) > 0:
        combined_df = pd.concat([existing_df, new_rows], ignore_index=True)
        combined_df.to_csv(STAGING_PATH, index=False)
        combined_df.to_excel('data/staging/dam_levels_staging.xlsx', index=False)
        print(f"\n[4] Appended new rows. Total rows: {len(combined_df):,}")
    else:
        print("\n[4] No new data to append — staging is up to date ✅")
else:
    # First time — full load
    print(" No existing staging found — performing full load...")
    long_df.to_csv(STAGING_PATH, index=False)
    long_df.to_excel('data/staging/dam_levels_staging.xlsx', index=False)
    print(f"\n[4] Full load complete. Rows saved: {len(long_df):,}")
